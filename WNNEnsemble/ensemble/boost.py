...
class AdaBoost(object):
    ...
    def executa_folds(self):
        for fold in self.dataset.folds:
            ...
            for n_learner, base_learner in enumerate(self.base_learners):
                ...
                self.treina_learner(fold, base_learner)
    ...
    def treina_learner(self, fold, base_learner):
        # samplear
...
class Bagging(object):
    ...
    def treina_learner(self, fold, base_learner, com_repeticao):
        ...
        tam_sample = (len(fold.inst_treino) * self.tam_treino)
        if com_repeticao:
            sample = self.sample_com_repeticao(fold.inst_treino, tam_sample)
        else:
            pesos = fold.retorna_pesos()
            sample = np.random.choice(fold.inst_treino, tam_sample, replace=False, p=pesos)
    ...
    def sample_com_repeticao(self, population, k):
